...
export const PortalDeliverablesView: React.FC<PortalDeliverablesProps> = ({
  ...
  const handleConfirmDownload = async () => {
    ...
    try {
      ...
      if (token && downloadModalItem.id && !downloadModalItem.id.startsWith('deliv-')) {
        ...
        if (rpcErr || !rpcData?.success) {
          alert(rpcData?.message || 'Payment Required: The project balance must be 100% fully paid before accessing deliverable packages.');
