    ...
    <div className="flex items-center justify-between gap-2 p-2 rounded-t-lg bg-zinc-900/90 border-b border-zinc-800 text-zinc-400 text-xs select-none flex-wrap">
      <div className="flex items-center gap-1 flex-wrap">
        <button
          type="button"
          onClick={() => onInsert('**', '**')}
          className="h-8 w-8 flex items-center justify-center rounded-sm hover:bg-zinc-800 hover:text-white transition-colors cursor-pointer"
          title="Bold (**text**)"
        >
      ...
      <div className="flex items-center gap-2">
        {onToggleFullscreen && (
          <button
            ...
            className={`h-8 w-8 flex items-center justify-center rounded-sm border transition-all cursor-pointer ${isFullscreen
        ...
        <button
          type="button"
          onClick={onCopy}
          className="h-8 flex items-center gap-1.5 px-3 rounded-sm bg-zinc-900/80 border border-zinc-800 hover:border-zinc-700 text-zinc-300 hover:text-white transition-all cursor-pointer text-xs font-medium"
        ...
        <div className="h-8 flex items-center rounded-sm bg-zinc-950/80 border border-zinc-800/80 p-0.5 font-mono text-xs">
          <button
            type="button"
            onClick={() => onViewModeChange?.('edit')}
            className={`h-7 px-3 rounded-sm transition-all cursor-pointer flex items-center justify-center ${viewMode === 'edit'
          ...
          <button
            ...
            className={`h-7 px-3 rounded-sm transition-all cursor-pointer flex items-center justify-center ${viewMode === 'split'
              ? 'bg-zinc-800 text-white font-semibold border border-zinc-700/60 shadow-sm'
              : 'text-zinc-400 hover:text-zinc-200'
              }`}
          >
          ...
          <button
            ...
            className={`h-7 px-3 rounded-sm transition-all cursor-pointer flex items-center justify-center ${viewMode === 'preview'
              ? 'bg-zinc-800 text-white font-semibold border border-zinc-700/60 shadow-sm'
              : 'text-zinc-400 hover:text-zinc-200'
              }`}
          >
