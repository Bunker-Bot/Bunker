...
export const TableOfContents: React.FC<TableOfContentsProps> = ({
  ...
  return (
    <div className="w-56 p-3 rounded-sm bg-zinc-900/60 border border-zinc-800/80 text-xs select-none shrink-0 font-mono hidden xl:block">
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full text-left truncate rounded px-2 py-1 transition-colors cursor-pointer text-[11px] ${item.level === 1
                  ? 'font-semibold'
                  : item.level === 2
                    ? 'pl-4 text-zinc-300'
                    : 'pl-6 text-zinc-400'
